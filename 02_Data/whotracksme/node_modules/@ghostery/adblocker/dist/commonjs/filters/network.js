...
exports.replaceOptionValueToRegexp = replaceOptionValueToRegexp;
...
class NetworkFilter {
    static parse(line, debug = false) {
        ...
        if (optionsIndex !== -1 && line.charCodeAt(optionsIndex + 1) !== 47 /* '/' */) {
            ...
            for (const rawOption of getFilterOptions(line, optionsIndex + 1, line.length)) {
                ...
                switch (option) {
                    ...
                    case 'content':
                        if (negation ||
                            (value.length === 0
                                ? (0, utils_js_1.getBit)(mask, NETWORK_FILTER_MASK.isException) === false
                                : replaceOptionValueToRegexp(value) === null) ||
                            optionValue !== undefined) {
                            return null;
                        }
                        mask = (0, utils_js_1.setBit)(mask, NETWORK_FILTER_MASK.isReplace);
                        optionValue = value;
                        break;
                    case 'removeparam':
                        // TODO: Support regex
                        ...
                        mask = (0, utils_js_1.setBit)(mask, NETWORK_FILTER_MASK.isRemoveParam);
                        optionValue = value;
                        break;
                    default: {
            ...
            if (domainsList !== undefined && domainsList.size !== 0) {
                domains = domains_js_1.Domains.parse(domainsList, {
                    delimiter: '|',
                    debug,
                });
                if (domains === undefined) {
            ...
            if (denyallowList !== undefined && denyallowList.size !== 0) {
                ...
                denyallow = domains_js_1.Domains.parse(denyallowList, {
                    delimiter: '|',
                    debug,
                });
                if (denyallow === undefined) {
        ...
        if (cptMaskPositive === 0) {
            mask = (0, utils_js_1.setBit)(mask, cptMaskNegative);
        }
        else if (cptMaskNegative === FROM_ANY) {
            mask = (0, utils_js_1.setBit)(mask, cptMaskPositive);
        }
        else {
            mask = (0, utils_js_1.setBit)(mask, cptMaskPositive & cptMaskNegative);
        }
    ...
    constructor({ filter, hostname, mask, domains, denyallow, optionValue, rawLine, regex, }) {
        ...
        this.mask = (0, utils_js_1.setBit)(mask, 0);
    ...
    isReplace() {
        return (0, utils_js_1.getBit)(this.getMask(), NETWORK_FILTER_MASK.isReplace);
    }
    isRemoveParam() {
        return (0, utils_js_1.getBit)(this.getMask(), NETWORK_FILTER_MASK.isRemoveParam);
    }
