...
exports.createLookupTokens = createLookupTokens;
 ...
 */
function createStylesheet(rules, style = cosmetic_js_1.DEFAULT_HIDING_STYLE) {
    if (rules.length === 0) {
        return '';
    }
 ...
 */
function createStylesheetFromRulesWithCustomStyles(rules, hidingStyle = cosmetic_js_1.DEFAULT_HIDING_STYLE) {
 ...
 */
function createStylesheetFromRules(rules, hidingStyle = cosmetic_js_1.DEFAULT_HIDING_STYLE) {
    const selectors = [];
    for (const rule of rules) {
...
class CosmeticFilterBucket {
    ...
    getStylesheetsFromFilters({ filters, extendedFilters, }, { getBaseRules, allowGenericHides, hidingStyle = cosmetic_js_1.DEFAULT_HIDING_STYLE, }) {
        let stylesheet = getBaseRules === false || allowGenericHides === false ? '' : this.getBaseStylesheet();
        if (hidingStyle !== cosmetic_js_1.DEFAULT_HIDING_STYLE) {
            stylesheet = stylesheet.replace(cosmetic_js_1.DEFAULT_HIDING_STYLE, hidingStyle);
        }
        if (filters.length !== 0) {
