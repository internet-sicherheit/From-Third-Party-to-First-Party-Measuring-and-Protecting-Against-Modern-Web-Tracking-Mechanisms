...
def visitLogNew(site_id, url, subpage_id):
    ...
    query="INSERT INTO visits_2 (site_id, url, subpage_id, visit_id, browser_id, start_time) values(" + str(site_id) + ", '"+ str(url) + "', "+ str(subpage_id) + ", '" + visit_id +  "', '"+ getMode() + "', '" + str(datetime.now()) + "')"
    print(query)
    db.exec(query)
...
def visitLogNew_Full(site_id, url, subpage_id, init_date, finish_date, timeout, state):
    db = DBOps()
    visit_id = str(site_id) + '_' + str(subpage_id)
    query="INSERT INTO visits_2 (site_id, url, subpage_id, visit_id, browser_id, start_time, finish_time, timeout, state) values(" + str(site_id) + ", '"+ str(url) + "', "+ str(subpage_id) + ", '" + visit_id +  "', '"+ getMode() + "', '" + str(init_date) + "', '" + str(finish_date) + "', '" + str(timeout) + "', " + str(state) + ")"
    print(query)
    db.exec(query)
...
def visitLogUpdate(site_id, subpage_id, state, dontChangeTime=False, timeout = 0):
    db = DBOps()
    if dontChangeTime:
        query= "UPDATE visits_2 SET state=" + str(state) + ", timeout=" + str(timeout) + " WHERE site_id=" + str(site_id) + " AND subpage_id=" + str( subpage_id) + " AND browser_id= '"+ getMode() + "'"
    else:
        query = "UPDATE visits_2 SET state=" + str(state) + ", finish_time='"+ str(datetime.now()) +"', timeout=" + str(timeout) + "  WHERE site_id=" + str(site_id) + " AND subpage_id=" + str( subpage_id) + " AND browser_id= '"+ getMode() + "'"
    db.exec(query)

def visitLog_ReqRes(visit_id, push_request, push_response):
    db = DBOps()
    query= "UPDATE visits_2 SET push_request = " + str(push_request) + ", push_response= " + str(push_response) + " WHERE visit_id='" + str(visit_id) + "' and browser_id='"+ getMode() +"'"
    db.exec(query)
