...
async def run_all_extractions(folder_paths, analyzed_info, cookies_classified):
    ...
    with ThreadPoolExecutor(max_workers=min(32, os.cpu_count() or 4)) as executor:
        for i, folder in tqdm(enumerate(folder_paths, start=1), total=len(folder_paths), desc=f'Pushing SQLITE to JSONL'):
            # Run the folder extraction in a thread
            folder_id, result = await loop.run_in_executor(executor, task, folder)

            # Flush immediately after this folder finishes
            flush_combined_data(i, result, [folder_id])
