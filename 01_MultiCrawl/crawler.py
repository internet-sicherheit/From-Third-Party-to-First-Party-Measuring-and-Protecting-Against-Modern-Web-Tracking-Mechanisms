...
SAVE_CONTENT = os.getenv("SAVE_CONTENT", "script") # Change to script to store JS
...
for i in range(NUM_BROWSERS):
    ...
    browser_params[i].tp_cookies = 'never' # add
    browser_params[i].channel_id = True # add
    browser_params[i].content_script_instrument = True # add
    browser_params[i].stealth_js_instrument = True # Add from extension
    if SAVE_CONTENT == "1":
...
manager_params.enable_storage = True # added
