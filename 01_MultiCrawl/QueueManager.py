...
from Ops import editLogQueue, lessMemory, list2str, getMemory, getSitesTableName
...
# Params
max_thread = 2  # getConfig('thread_count')
report = ''
# vars
...
def findSuspended():
    ...
    query = "UPDATE  " + getSitesTableName() + "  SET state_" + getMode() + "=-1  WHERE state_" + \
        getMode() + "=0 " + query_and
    db.exec(query)
...
def siteWaitForMe():
    ...
    query_and = getQueryElements(True)
    # query = "SELECT id, concat('http://',site), subpages FROM  " + getSitesTableName() + "  WHERE site_state='in_queue' AND state_" +
    #  getMode() + " = -1 " + query_and + " ORDER BY id ASC LIMIT 1"   # TABLE sites
    query = "SELECT id, concat(scheme, site), subpages FROM  " + getSitesTableName() + "  WHERE site_state='in_queue' AND state_" + \
...
def pullNextSite():
    ...
    # query = "SELECT id, concat('http://',site), subpages FROM  " + getSitesTableName() + "  WHERE site_state IS NULL AND state_" + \
    #        getMode() + " = -1  ORDER BY id ASC LIMIT 1"    #SITE sites
    query = "SELECT id, concat(scheme, site), subpages FROM  " + getSitesTableName() + "  WHERE site_state IS NULL AND state_" + \
            getMode() + " = -1 "+isASC_or_Desc()+" LIMIT 1"
    rows = db.select(query)
    if len(rows) > 0:
        site_ID = str(rows[0][0])
        query = "UPDATE  " + getSitesTableName() + "  SET site_state ='in_queue', state_" + getMode() + \
            " = 0 WHERE id=" + site_ID
        db.exec(query)
        q_item = QueueItem(rows[0][0], rows[0][1], None,
                           rows[0][2], None, None, None)
...
def queueManager(p_q_item, p_OP):
    ...
    if p_OP == 'waiting':
        ...
        query = "UPDATE  " + getSitesTableName() + "  SET state_" + getMode() + " = 0 WHERE id=" + \
            str(p_site_id)
        db.exec(query)

        query = "SELECT 1 FROM  " + getSitesTableName() + "  WHERE  " + "  state_" + getMode() + " = 0   AND id=" + \
            str(p_site_id)
        rows = db.select(query)
        
        if len(rows) != 0:
            query = "UPDATE  " + getSitesTableName() + "  SET ready=true WHERE id=" + \
                str(p_site_id)
            db.exec(query)
...
def vpn_Connected():
    ...
    try:
        ...
        if s.startswith("openwpm_native_us"):
            c_code = 'US'
        if s.startswith("openwpm_native_eu"):
            c_code = 'DE'
...
def restart():
    ...
    try:
        ...
        os.execl('/home/' + getpass.getuser() + '/miniforge3/envs/openwpm/bin/python3',
                 os.path.abspath(__file__), *sys.argv)
        print('CANT RESTART!')


def isSiteReady(p_site_id):
    query = "SELECT 1 FROM  " + getSitesTableName() + "  WHERE ready IS TRUE and id=" + \
        str(p_site_id)
    rows = db.select(query)
    if len(rows) > 0:
        return True
...
def changeSiteState(siteID, state, timeout=0):
    print('changeSiteState: ', str(siteID), str(state), str(timeout))

    query = "UPDATE  " + getSitesTableName() + "  SET state_{} = {}, timeout={}  where id = {} ".format(
...
def processStartedSites():
    ...
    for item in crawlers:
        ...
        if minutes > getConfig('timeout_site_min'):
            ...
            db = DBOps()
            state = db.select('select state_' + getMode() + ' from  ' + getSitesTableName() + '  where id=' + str(site_id))[0][0]
            if state == 1:
...
def processQueue2():
    ...
    if total_items >= getConfig('thread_count'):
        ...
        # get  " + getSitesTableName() + "  wait for me
        query_and = getQueryElements2(in_queue)
        query = "SELECT id FROM  " + getSitesTableName() + "  WHERE site_state='in_queue' AND state_" + \
            getMode() + " = -1 " + query_and + isASC_or_Desc()+" LIMIT 1"
        rows = db.select(query)
        ...
        if len(rows) > 0:
            ...
            query = "SELECT id FROM  " + getSitesTableName() + "  WHERE site_state IS NULL AND state_" + \
                    getMode() + " = -1  " + query_and + isASC_or_Desc()+" LIMIT 1"
...
def startNewCrawler(id):
    ...
    subprocess.Popen(['/home/' + getpass.getuser() + '/miniforge3/envs/openwpm/bin/python3',
