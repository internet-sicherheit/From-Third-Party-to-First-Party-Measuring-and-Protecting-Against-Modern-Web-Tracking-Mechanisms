...
from Ops import addTime2Datetime, delFolder, isThirdParty, terminateProcessBySiteID, visitLogNew, visitLogNew_Full, getSitesTableName
    ...
    try:
        ...
        for r in rows:
            ...
            visit.incomplete_visit = getIncompleteVisits(
                root_site_id, visit_id_openwpm, subpage_id)
            visit.javascript = getJavascript( # Remove comment sst
                root_site_id, visit_id_openwpm, subpage_id)
...
def runOpenWPMInstance(p_queue, p_queue_item):
    ...
    try:
        ...
        #delFolder(getProfileFolder(root_site_id))  --- removed to keep the scripts
...
if __name__ == "__main__":
    ...
    sql_select="SELECT id,  concat(scheme, site), subpages from  " + getSitesTableName() + "  where id= "+str(sys.argv[1])
    print(sql_select)
    r = db.select(sql_select)[0]
