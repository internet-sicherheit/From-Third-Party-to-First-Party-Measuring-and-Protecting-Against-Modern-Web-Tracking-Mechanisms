...
from custom_command import InstallExtension
...
for browser_param in browser_params:
    ...
    # Enable Screenshots
    browser_param.take_screenshot = True
...
if args.analyse:
    db_name = "./datadir/crawl-data-analysis.sqlite"
else:
    db_name = "./datadir/crawl-data.sqlite"

# Commands time out by default after 60 seconds
with TaskManager(
    manager_params,
    browser_params,
    SQLiteStorageProvider(Path(db_name)),
    None,
) as manager:
    # Visits the sites
    for index, site in enumerate(sites):
        ...
        etld = str(index) + "-" + ext.domain + "." + ext.suffix
        ...
        # Take a screenshot
        #command_sequence.append_command(command_sequence.screenshot_full_page(f"screenshot_{index}_{site}"))
        #command_sequence.append_command(command_sequence.save_screenshot(f"screenshot_{index}_{site}"))


        # 1) Viewport screenshot (visible area)
        #command_sequence.save_screenshot(suffix="viewport_after_load")

        # 2) Full-page screenshot (scroll + stitch)
        command_sequence.screenshot_full_page(suffix=etld)
